...
class CapabilityResolver:
    ...
    def rank(self, request: CapabilityRequest, *, world: Any | None = None) -> tuple[CapabilityResolution, ...]:
        ...
        for adapter in self.registry:
            ...
            product_fraction = len(matched_products) / len(requested_products) if requested_products else 0.5
            modality_fraction = len(matched_modalities) / len(requested_modalities) if requested_modalities else 0.0
