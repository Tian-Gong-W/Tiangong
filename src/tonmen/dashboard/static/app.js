(() => {
  ...
  function renderMission() {
    ...
    $("#mission-steps").innerHTML = m.steps.map((s,i) => `<div class="step"><div class="step-index">${String(i+1).padStart(2,"0")}</div><div><strong>${esc(s.tool)}</strong><small>${esc(s.rationale || s.target)}</small></div><span class="step-state ${esc(s.state)}">${esc(stateMark[s.state] || "")} ${esc(stateLabel[s.state] || s.state)}</span></div>`).join("");
    ...
    }).join("") : `<div class="muted">原始 Evidence 已保存，但目前沒有可確定解析的 Intelligence Fact。</div>`;
  ...
  function renderChronicle(m) {
    ...
    m.evidence.forEach(e => events.push({time:e.finished_at,text:`${e.tool} 執行完成 · exit ${e.exit_code}`,tone:e.exit_code === 0 ? "" : "danger"}));
  ...
  $("#refresh-btn").addEventListener("click",()=>refreshAll()); $("#mission-select").addEventListener("change",e=>loadMission(e.target.value));
  ...
  refreshAll();
  setInterval(async () => { if (document.hidden) return; try { const ms=await api("/api/missions"); state.missions=ms.missions; renderMissionSelect(); if(state.current) await loadMission(state.current.id); } catch (_) {} }, 5000);
})();
