...
def mission_payload(plan, run) -> dict[str, Any]:
    ...
    return {
        ...
        "observations": [
            {"id": item.id, "source": item.source, "target": item.target, "summary": item.summary, "evidence_id": item.evidence_id, "captured_at": _iso(item.captured_at), "metadata": dict(item.metadata)}
            for item in run.observations
        ],
        "evidence": [
            {"id": item.id, "tool": item.tool, "target": item.target, "argv": list(item.argv), "exit_code": item.exit_code, "stdout": item.stdout, "stderr": item.stderr, "started_at": _iso(item.started_at), "finished_at": _iso(item.finished_at)}
...
class DashboardState:
    ...
    def scope(self) -> dict[str, Any]:
        with self._lock:
            return {"allowed": [{"rule": rule, "default": rule in DEFAULT_ALLOWED_TARGETS} for rule in self.config.allowed_targets], "denied": list(self.config.denied_targets)}
    ...
    def missions(self) -> list[dict[str, Any]]:
        with self._lock:
            return [{"id": entry.run_id, "plan_id": entry.plan_id, "target": entry.target, "state": entry.state.value, "started_at": _iso(entry.started_at), "finished_at": _iso(entry.finished_at)} for entry in self.chronicle.list()]
    ...
    def approve_mission(self, run_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            result = MissionLoop(self.runtime).resume(plan, run, approval_tokens={waiting.id: grant.token})
            self.chronicle.save(plan, result.run)
            payload = mission_payload(plan, result.run)
            payload["stop_reason"] = result.stop_reason.value
            return payload
    ...
    def reason(self, run_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            return {"id": decision.id, "action": decision.action.value, "summary": decision.summary, "basis_fact_ids": list(decision.basis_fact_ids), "next_step_id": decision.next_step_id, "requires_human": decision.requires_human}
...
class TonmenDashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            if path == "/":
                html = self._asset("index.html").decode("utf-8").replace("__TONMEN_CSRF__", self.server.csrf_token)
                self._send_bytes(200, "text/html; charset=utf-8", html.encode("utf-8")); return
            if path == "/assets/app.css":
                self._send_bytes(200, "text/css; charset=utf-8", self._asset("app.css"), cache="public, max-age=300"); return
            if path == "/assets/app.js":
                self._send_bytes(200, "text/javascript; charset=utf-8", self._asset("app.js"), cache="public, max-age=300"); return
