...
TONMEN does **not** put raw tool `argv`, raw `stdout`, or raw `stderr` into the local advisory context.

The local model returns structured JSON only. Any Fact ID returned by the model is filtered against Fact IDs that already exist in the mission graph.
...
- a challenge to the current analytical interpretation;
- basis Fact IDs.
