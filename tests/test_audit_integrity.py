...
def test_audit_events_form_a_verifiable_sha256_chain(tmp_path):
...
def test_audit_tampering_is_detected_and_blocks_future_append(tmp_path):
    ...
    assert "hash" in (verification.error or "")
...
def test_first_chained_event_anchors_entire_legacy_prefix(tmp_path):
    path = tmp_path / "audit.jsonl"
    legacy_rows = [
        {
            "id": "legacy-1",
