...
class StreamingAdapter(ToolAdapter):
    spec = ToolSpec(
        name="stream-demo",
        category="test",
        description="streaming test adapter",
...
def test_executor_streams_stdout_stderr_events_with_context():
    ...
    registry.register(StreamingAdapter())
    ...
    outcome = executor.execute(
        ToolRequest(
            tool="stream-demo",
            target="local-test",
            context={"mission_id": "mission-1", "step_id": "step-1"},
        )
    )
    ...
    assert all(event.data["mission_id"] == "mission-1" for event in output)
...
def test_mission_loop_publishes_semantic_lifecycle_events(tmp_path):
    ...
    types = [event.type for event in bus.read_after(0)]
