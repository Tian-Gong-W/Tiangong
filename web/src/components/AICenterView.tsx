...
export const AICenterView: React.FC<AICenterViewProps> = ({ data, lead, onSave, onProbe, onSaveKey, onClearKey }) => {
  ...
  const saveLeadSettings = () => act('settings', async () => {
    const values: Record<string, unknown> = { lead_enabled: enabled, pool };
  ...
  return (
    <div className="flex-1 p-6 md:p-8 overflow-y-auto text-slate-100 space-y-5 max-w-6xl mx-auto">
      <div className="border-b border-slate-800 pb-4">
        ...
        <p className="text-xs text-slate-400 mt-1">显示 Tiangong ProviderHub 返回的实际配置、连接和历史用量</p>
      </div>
      ...
      <div className="p-5 rounded-2xl bg-slate-900/85 border border-slate-800 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div className="text-xs text-slate-400">
            当前状态
            <div className="mt-1.5 rounded-xl bg-slate-950 border border-slate-800 px-3 py-2.5 font-mono text-slate-200">
              {String(lead.provider || (enabled ? 'provider 未配置' : 'disabled'))} · {lead.key_configured ? 'Key 已配置' : 'Key 未配置'} · {model || '模型未配置'}
            </div>
          </div>
        </div>

        {lead.error && <div className="rounded-xl border border-amber-500/20 bg-amber-950/20 px-3 py-2 text-[11px] text-amber-300">{String(lead.error)}</div>}
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {providers.map((provider: any) => {
          ...
          const ready = Boolean(provider.last_probe?.ready);
          return (
            <div key={provider.id} className="p-5 rounded-2xl bg-slate-900/85 border border-slate-800 space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-bold text-white">{provider.label || provider.id}</div>
                  <div className="text-[11px] text-slate-500 font-mono mt-1">{provider.transport || '—'} · {provider.default_model || '—'}</div>
                </div>
                <span className={`text-[10px] font-bold ${ready ? 'text-emerald-400' : configured ? 'text-cyan-300' : 'text-amber-400'}`}>
                  {ready ? 'READY' : configured ? 'CONFIGURED' : 'NOT CONFIGURED'}
                </span>
              </div>
              ...
              {provider.last_probe?.detail && <div className="text-[11px] text-slate-500 rounded-lg bg-slate-950 px-3 py-2">{String(provider.last_probe.detail)}</div>}
      ...
      <div className="text-[11px] text-slate-500 flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5" />浏览器不读取已保存的密钥值，API 仅返回配置状态。</div>
    </div>
  );
};
